...
static void log_uart_tx(char *ptr,int len)
{
    while (len)
    {
        if (REG_FIELD_RD(UART3->SR, UART_SR_TFNF))
        {
            len--;
            UART3->TBR = (*ptr++ & (uint8_t)0xFF);
        }
    }
}
...
static void log_uart_init()
{
    ...
    REG_FIELD_WR(UART3->LCR, UART_LCR_BRWEN, 0);
    UART3->FCR = UART_FCR_TFRST_MASK | UART_FCR_RFRST_MASK | UART_FCR_FIFOEN_MASK;
    UART3->LCR = FIELD_BUILD(UART_LCR_DLS, LOG_UART_WORDLENGTH) |
