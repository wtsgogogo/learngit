...
{
    while((ECC->STAT&ECC_DONE_MASK)==0);
}

/*
void h_calc(const uint32_t *p,uint32_t *result_h)
{
    memcpy32((uint32_t *)ECC->NRAM,p,8);
    ECC->CON = FIELD_BUILD(ECC_RUN,1)|FIELD_BUILD(ECC_MODE,Mode_H);
    calc_done_wait();
    memcpy32(result_h,(uint32_t *)&ECC->ARAM[56],8);
}
...
{
    ECC->CON = FIELD_BUILD(ECC_RUN,1)|FIELD_BUILD(ECC_OPR,dst)|FIELD_BUILD(ECC_RES,dst)|FIELD_BUILD(ECC_MODE,Mode_MontIn);
}

static void mont_in_data(const uint32_t *data,uint8_t dst)
{
    memcpy32((uint32_t *)&ECC->ARAM[dst*8],data,8);
    mont_in(dst);
}

static void p_in()
{
    memcpy32((uint32_t *)ECC->NRAM,ecc_curve->p,8);
}

static void h_in()
{
    memcpy32((uint32_t *)&ECC->ARAM[56],ecc_curve->h,8);
}
...
static void dblu()
{
    ECC->CON = FIELD_BUILD(ECC_RUN,1)|FIELD_BUILD(ECC_COMB,1)|FIELD_BUILD(ECC_COMMOD,ECC_DBLU);
}

static void zacau_calc(bool remap)
{
    ECC->CON = FIELD_BUILD(ECC_RUN,1)|FIELD_BUILD(ECC_COMB,1)|FIELD_BUILD(ECC_REMAP,remap?1:0)|FIELD_BUILD(ECC_COMMOD,ECC_ZACAU);
}
...
{
    ECC->CON = FIELD_BUILD(ECC_RUN,1)|FIELD_BUILD(ECC_COMB,1)|FIELD_BUILD(ECC_REMAP,remap?1:0)|FIELD_BUILD(ECC_COMMOD,ECC_ZADDC);
}

static void mont_out(uint8_t src)
{
    ECC->CON = FIELD_BUILD(ECC_RUN,1)|FIELD_BUILD(ECC_OPR,src)|FIELD_BUILD(ECC_RES,src)|FIELD_BUILD(ECC_MODE,Mode_MontOut);
}

static void inversion_calc()
{
    uint8_t i;
    memcpy32((uint32_t *)&ECC->ARAM[0],ecc_curve->p,8);
    for(i=0;i<8;++i)
    {
        ECC->ARAM[48+i] = 0;
        ECC->ARAM[56+i] = 0;
    ...
    ECC->ARAM[48] = 1;
    ECC->CON = FIELD_BUILD(ECC_RUN,1)|FIELD_BUILD(ECC_RES,3)|FIELD_BUILD(ECC_MODE,Mode_ModInver);
}
...
{
    ECC->CON = FIELD_BUILD(ECC_RUN,1)|FIELD_BUILD(ECC_COMB,1)|FIELD_BUILD(ECC_REMAP,remap?1:0)|FIELD_BUILD(ECC_COMMOD,ECC_ZADDU);
}
...
{
    copy_from_eccram((uint32_t *)&ECC->ARAM[8],ecc_result[0]);
    copy_from_eccram((uint32_t *)&ECC->ARAM[16],ecc_result[1]);
}
...
{
    HAL_ECC_MSP_INT_CLRPENDING();
    calc_init(curve,private_key,public_key,result);
    calc_fsm_run();
    HAL_ECC_MSP_INT_ENABLE();
}

__attribute__((weak)) void HAL_ECC_PointMult_Complete_Callback(void){}

void ECC_IRQHandler(void)
{
    HAL_ECC_MSP_INT_DISABLE();
    ECC->CON = FIELD_BUILD(ECC_RST,1);
    if(ecc_fsm == FSM_DONE)
    {
        result2buffer();
        HAL_ECC_PointMult_Complete_Callback();
    }else
    {
        calc_fsm_run();
        HAL_ECC_MSP_INT_ENABLE();
