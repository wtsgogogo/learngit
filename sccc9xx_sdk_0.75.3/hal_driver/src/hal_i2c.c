...
HAL_StatusTypeDef HAL_I2C_Init(I2C_HandleTypeDef *hi2c)
{
    ...
    ASSERT(IS_I2C_ALL_INSTANCE(hi2c->Instance));
    ASSERT(IS_I2C_CLOCK_SPEED(hi2c->Init.ClockSpeed));
    ASSERT(IS_I2C_OWN_ADDRESS1(hi2c->Init.OwnAddress1));
    ASSERT(IS_I2C_ADDRESSING_MODE(hi2c->Init.AddressingMode));
    ASSERT(IS_I2C_DUAL_ADDRESS(hi2c->Init.DualAddressMode));
    ASSERT(IS_I2C_OWN_ADDRESS2(hi2c->Init.OwnAddress2));
    ASSERT(IS_I2C_GENERAL_CALL(hi2c->Init.GeneralCallMode));
    ASSERT(IS_I2C_NO_STRETCH(hi2c->Init.NoStretchMode));
...
HAL_StatusTypeDef HAL_I2C_Slave_Transmit_IT(I2C_HandleTypeDef *hi2c, uint8_t *pData, uint8_t Size)
{
    if (hi2c->State == HAL_I2C_STATE_READY)
    {
        ...
        #ifndef SC208
        /* Clear TXE irq on platforms except for SC208 */
