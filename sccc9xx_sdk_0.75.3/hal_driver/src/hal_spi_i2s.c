...
#ifdef SC208
static void sc208_rx_load_data_16bit(SPI_HandleTypeDef *hspi)
{
...
static void sc208_8bit_packing_mode_handle(SPI_HandleTypeDef *hspi)
{
    uint8_t i = REG_FIELD_RD(hspi->Instance->SR, SPI_SR_RXFLV)/2;
    while (i--)
    {
        sc208_rx_load_data_16bit(hspi);
    }
...
static void sc208_8bit_load_rx_dummy_data(SPI_HandleTypeDef *hspi)
{
    uint8_t i = REG_FIELD_RD(hspi->Instance->SR, SPI_SR_RXFLV)/2;
    while (i--)
    {
...
static void spi_config(SPI_HandleTypeDef *hspi, bool itmode)
{
    ...
    else  // 8 Bit Mode
    {
        ...
        #ifdef SC208
        hspi->Rx_Env.Interrupt.transfer_Fun = hspi->Rx_Env.Interrupt.pBuffPtr? sc208_8bit_packing_mode_handle: sc208_8bit_load_rx_dummy_data;
        #else 
