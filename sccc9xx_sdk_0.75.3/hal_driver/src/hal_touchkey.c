...
{
    HAL_TOUCHKEY_MSP_Init();
    return HAL_OK;
}

HAL_StatusTypeDef HAL_TOUCHKEY_DeInit()
{
    HAL_TOUCHKEY_MSP_DeInit();
    return HAL_OK;
}

HAL_StatusTypeDef HAL_TOUCHKEY_SetParam(TOUCHKEY_PARAM *param)
{
    TOUCHKEY->TOUCHKEY_CLK_CFG = FIELD_BUILD(TOUCHKEY_HPRD, param->clk_hprd) |
                                   FIELD_BUILD(TOUCHKEY_LPRD, param->clk_lprd);
    TOUCHKEY->TOUCHKEY_SCAN_MTIM = FIELD_BUILD(TOUCHKEY_SCAN_MTIM, param->scan_mtim);
    TOUCHKEY->TOUCHKEY_SCAN_CFG = FIELD_BUILD(TOUCHKEY_CH_EN, param->scan_channel_en) |
    ...
    TOUCHKEY->TOUCHKEY_ANA_CFG = FIELD_BUILD(TOUCHKEY_CP_VCTL, param->touchkey_cp_vctl) |
...
{
    TOUCHKEY->INTR_MSK = 0;
    TOUCHKEY->INTR_CLR = TOUCHKEY_INTR_END_MASK | TOUCHKEY_INTR_ERR_MASK;
    REG_FIELD_WR(TOUCHKEY->TOUCHKEY_START, TOUCHKEY_START, 1);
    while (!(TOUCHKEY->INTR_RAW & TOUCHKEY_INTR_END_MASK))
        ;
    REG_FIELD_WR(TOUCHKEY->INTR_CLR, TOUCHKEY_INTR_END, 1);
    uint32_t sccfg = TOUCHKEY->TOUCHKEY_SCAN_CFG;
    uint32_t *ptr = (uint32_t *)&TOUCHKEY->CH0_CNT;
...
{
    TOUCHKEY->INTR_MSK = TOUCHKEY_INTR_END_MASK | TOUCHKEY_INTR_ERR_MASK;
    TOUCHKEY->INTR_CLR = TOUCHKEY_INTR_END_MASK | TOUCHKEY_INTR_ERR_MASK;
    REG_FIELD_WR(TOUCHKEY->TOUCHKEY_START, TOUCHKEY_START, 1);
    return HAL_OK;
}
...
{
    uint32_t iflag = TOUCHKEY->INTR_STT;
    ...
    {
        REG_FIELD_WR(TOUCHKEY->INTR_CLR, TOUCHKEY_INTR_ERR, 1);
