...
{
    HAL_WWDG_MSP_Init();
    WWDG->WWDG_LOCK = 0x1ACCE551;
    WWDG->WWDG_LOAD = LoadValue;
    WWDG->WWDG_CON = FIELD_BUILD(REG_WWDG_CLKS,1) |
...
{
    REG_FIELD_WR(WWDG->WWDG_CON, REG_WWDG_EN, 0);
    HAL_WWDG_MSP_DeInit();
    return HAL_OK;
}
