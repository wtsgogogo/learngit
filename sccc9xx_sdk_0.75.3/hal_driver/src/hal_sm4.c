...
{    
    SM4->SM4_KEY0 = u8tou32(key);
    SM4->SM4_KEY1 = u8tou32(key + 4);
    SM4->SM4_KEY2 = u8tou32(key + 8);
    SM4->SM4_KEY3 = u8tou32(key + 12);
    SM4->SM4_START = SM4_KEY_START_MASK;
}

HAL_StatusTypeDef HAL_SM4_KeyExpansion(const uint8_t *key)
{   
    sm4_key_start(key);
    while (REG_FIELD_RD(SM4->INTR_RAW, SM4_KEY_END) == 0)
        ;
    SM4->INTR_CLR = SM4_KEY_END_MASK;
    return HAL_OK;
}

HAL_StatusTypeDef HAL_SM4_KeyExpansion_IT(const uint8_t *key)
{
    SM4->INTR_MSK |= SM4_KEY_END_MASK;
    sm4_key_start(key);
    return HAL_OK;
}
...
static void sm4_data_length_inout_config(const uint8_t *data, uint32_t length, uint8_t *result)
{
    ...
    crypt_in = data;
    crypt_out = result;
    SM4->SM4_CTRL |= (uint8_t)(length / 16 - 1) << SM4_CALC_LEN_POS;
}

static void sm4_start()
{
    SM4->CALC_WRD = u8tou32(crypt_in);
    SM4->CALC_WRD = u8tou32(crypt_in + 4);
    SM4->CALC_WRD = u8tou32(crypt_in + 8);
    SM4->CALC_WRD = u8tou32(crypt_in + 12);
}

static void sm4_end()
{
    u32tou8(SM4->CALC_RSLT0, crypt_out);
    u32tou8(SM4->CALC_RSLT1, crypt_out + 4);
    u32tou8(SM4->CALC_RSLT2, crypt_out + 8);
    u32tou8(SM4->CALC_RSLT3, crypt_out + 12);
    crypt_in += 16;
    crypt_out += 16;
}
...
{
    SM4->SM4_START = SM4_CALC_START_MASK;
    do
    {
        while (REG_FIELD_RD(SM4->INTR_RAW, SM4_INTR_DATA) == 1)
        {
            if (writeflag)
            {
                sm4_start();
            ...
            writeflag = !writeflag;
            SM4->INTR_CLR = SM4_INTR_DATA_MASK;
        }
    } while (REG_FIELD_RD(SM4->INTR_RAW, SM4_INTR_END) == 0);
    SM4->INTR_CLR = SM4_INTR_END_MASK;
    sm4_end();
}
...
HAL_StatusTypeDef HAL_SM4_Encrypt(const uint8_t *data, uint8_t *result, uint32_t length)
{
    ...
    REG_FIELD_WR(SM4->SM4_CTRL,SM4_CALC_DEC,0);
    sm4_calculate();
    return HAL_OK;
}
...
{
    sm4_data_length_inout_config(data, length, result);
    REG_FIELD_WR(SM4->SM4_CTRL,SM4_CALC_DEC,1);
    sm4_calculate();
    return HAL_OK;
}
...
HAL_StatusTypeDef HAL_SM4_Encrypt_IT(const uint8_t *data, uint8_t *result, uint32_t length)
{
    sm4_data_length_inout_config(data, length, result);
    REG_FIELD_WR(SM4->SM4_CTRL,SM4_CALC_DEC,0);
    MODIFY_REG(SM4->INTR_MSK, SM4_INTR_DATA_MASK | SM4_INTR_END_MASK, 1 << SM4_INTR_DATA_POS | 1 << SM4_INTR_END_POS);
    SM4->SM4_START = SM4_CALC_START_MASK;
    return HAL_OK;
}
...
{
    if (REG_FIELD_RD(SM4->INTR_STT, SM4_INTR_DATA))
    {
        if (writeflag)
        {
            writeflag = !writeflag;
    ...
    if (REG_FIELD_RD(SM4->INTR_STT, SM4_INTR_END))
    {
        sm4_end();
        SM4->INTR_CLR = SM4_INTR_END_MASK;
        HAL_SM4_Calculation_Complete_Callback(!REG_FIELD_RD(SM4->SM4_CTRL,SM4_CALC_DEC));
    }
    if(REG_FIELD_RD(SM4->INTR_STT, SM4_KEY_END))
    {
        SM4->INTR_CLR = SM4_KEY_END_MASK;
