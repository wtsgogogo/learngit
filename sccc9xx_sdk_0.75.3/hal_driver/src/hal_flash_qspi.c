...
void XIP_BANNED_FUNC(hal_flash_xip_start,)
{
    ...
    struct qspi_direct_read_config_param direct_read_param; //do not initialize this variable with a const struct
    ...
    qspi_direct_read_config(&direct_read_param);
...
{
    qspi_stig_send_command(WRITE_ENABLE_OPCODE);
}
...
static void flash_program_param_set(uint32_t offset,uint8_t *data,uint16_t length,uint8_t multi_type,struct qspi_stig_rd_wr_param *param)
{
...
void XIP_BANNED_FUNC(do_hal_flash_write_reg_func,void *param)
{
    ...
    qspi_stig_write_register(ptr->opcode,ptr->buf,ptr->length);
}

void XIP_BANNED_FUNC(do_hal_flash_chip_erase_func,void *param)
{
    qspi_stig_send_command(CHIP_ERASE_OPCODE);
}
...
{
    qspi_stig_send_command( DEEP_POWER_DOWN_OPCODE);
}

void XIP_BANNED_FUNC(hal_flash_release_from_deep_power_down,)
{
    qspi_stig_send_command(RELEASE_FROM_DEEP_POWER_DOWN_OPCODE);
}

void XIP_BANNED_FUNC(do_hal_flash_read_reg_func,void *param)
{
    struct flash_wr_rd_reg_param *ptr = param;
    qspi_stig_read_register(ptr->opcode,ptr->buf,ptr->length);
}
...
{
    qspi_stig_send_command(RESET_EN_OPCODE);
    qspi_stig_send_command(RESET_OPCODE);
}

void XIP_BANNED_FUNC(hal_flash_prog_erase_suspend,)
{
    qspi_stig_send_command(PROG_ERASE_SUSPEND);
}


void XIP_BANNED_FUNC(hal_flash_prog_erase_resume,)
{
    qspi_stig_send_command(PROG_ERASE_RESUME);
}
...
void XIP_BANNED_FUNC(hal_flash_xip_mode_reset,)
{
    ...
    qspi_stig_write_register(CONTINUOUS_READ_MODE_RESET,&data,sizeof(data));
}
