...
static void XIP_BANNED_FUNC(hal_flash_quad_xip_mode_enter,)
{
    ...
    RAM_ASSERT(dummy == DUMMY_BYTE_VAL);
    #endif
}
...
ROM_SYMBOL void XIP_BANNED_FUNC(hal_flash_xip_start,)
{
    ...
    if(hal_flash_dual_mode_get())
    {
        ...
        qspiv2_direct_dual_read_config();
    }else
    {
        hal_flash_quad_xip_mode_enter();
        qspiv2_direct_quad_read_config();
    } 
...
{
    qspiv2_stg_send_command(WRITE_ENABLE_OPCODE);
}

ROM_SYMBOL void XIP_BANNED_FUNC(hal_flash_xip_mode_reset,)
{
    uint8_t data = CONTINUOUS_READ_MODE_RESET;
    qspiv2_stg_write_register(CONTINUOUS_READ_MODE_RESET,&data,sizeof(data));
}

ROM_SYMBOL void XIP_BANNED_FUNC(do_hal_flash_prog_func,void *param)
{
    qspiv2_stg_read_write(param);
}

ROM_SYMBOL void XIP_BANNED_FUNC(do_hal_flash_write_reg_func,void *param)
{
    struct flash_wr_rd_reg_param *ptr = param;
    qspiv2_stg_write_register(ptr->opcode,ptr->buf,ptr->length);
}

static void flash_program_param_set(uint32_t offset,uint8_t *data,uint16_t length,uint8_t multi_type,struct qspiv2_stg_cfg *cfg)
{
    cfg->ctrl.sw_cyc = 31;
    cfg->ctrl.sw_en = 1;
...
{
    qspiv2_stg_send_command(CHIP_ERASE_OPCODE);
}
...
static void hal_flash_read_loop(struct qspiv2_stg_cfg *cfg,uint8_t opcode,uint32_t offset,uint16_t length)
{
    while(length)
    {
        cfg->ca_high = opcode << 24 | offset;
...
{
    qspiv2_stg_send_command( DEEP_POWER_DOWN_OPCODE);
}

ROM_SYMBOL void XIP_BANNED_FUNC(hal_flash_release_from_deep_power_down,)
{
    qspiv2_stg_send_command(RELEASE_FROM_DEEP_POWER_DOWN_OPCODE);
}

ROM_SYMBOL void XIP_BANNED_FUNC(do_hal_flash_read_reg_func,void *param)
{
    struct flash_wr_rd_reg_param *ptr = param;
    qspiv2_stg_read_register(ptr->opcode,ptr->buf,ptr->length);
}
...
{
    qspiv2_stg_send_command(RESET_EN_OPCODE);
    qspiv2_stg_send_command(RESET_OPCODE);
}

ROM_SYMBOL void XIP_BANNED_FUNC(hal_flash_prog_erase_suspend)
{
    qspiv2_stg_send_command(PROG_ERASE_SUSPEND);
}

ROM_SYMBOL void XIP_BANNED_FUNC(hal_flash_prog_erase_resume)
{
    qspiv2_stg_send_command(PROG_ERASE_RESUME);
}
