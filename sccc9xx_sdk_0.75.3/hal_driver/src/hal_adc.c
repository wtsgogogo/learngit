...
HAL_StatusTypeDef HAL_ADC_Init(ADC_HandleTypeDef *hadc)
{
 ...
 //   ASSERT(IS_ADC_ALL_INSTANCE(hadc->Instance));
    ASSERT(IS_ADC_DATA_ALIGN(hadc->Init.DataAlign));
    ASSERT(IS_ADC_SCAN_MODE(hadc->Init.ScanConvMode));
    ASSERT(IS_FUNCTIONAL_STATE(hadc->Init.ContinuousConvMode));
    ASSERT(IS_VREF_SWITCH(hadc->Init.Vref));
 //   ASSERT(IS_ADC_EXTTRIG(hadc->Init.ExternalTrigConv));

  if(hadc->Init.ScanConvMode != ADC_SCAN_DISABLE)
  {
    ASSERT(IS_ADC_REGULAR_NB_CONV(hadc->Init.NbrOfConversion));
    ASSERT(IS_FUNCTIONAL_STATE(hadc->Init.DiscontinuousConvMode));
    if(hadc->Init.DiscontinuousConvMode != DISABLE)
    {
      ASSERT(IS_ADC_REGULAR_DISCONT_NUMBER(hadc->Init.NbrOfDiscConversion));
    }
  }
...
HAL_StatusTypeDef HAL_ADC_PollForEvent(ADC_HandleTypeDef* hadc, uint32_t EventType, uint32_t Timeout)
{
  ...
  /* Check the parameters */
  //ASSERT(IS_ADC_ALL_INSTANCE(hadc->Instance));
  ASSERT(IS_ADC_EVENT_TYPE(EventType));
...
uint32_t HAL_ADCEx_InjectedGetValue(ADC_HandleTypeDef* hadc, uint32_t InjectedRank)
{
  ...
  /* Check the parameters */
  //ASSERT(IS_ADC_ALL_INSTANCE(hadc->Instance));
  ASSERT(IS_ADC_INJECTED_RANK(InjectedRank));
...
HAL_StatusTypeDef HAL_ADC_ConfigChannel(ADC_HandleTypeDef *hadc, ADC_ChannelConfTypeDef *sConfig)
{
    ...
    /* Check the parameters */
    //ASSERT(IS_ADC_ALL_INSTANCE(hadc->Instance));
    ASSERT(IS_ADC_CHANNEL(sConfig->Channel));
    ASSERT(IS_ADC_REGULAR_RANK(sConfig->Rank));
    ASSERT(IS_ADC_SAMPLE_TIME(sConfig->SamplingTime));
...
HAL_StatusTypeDef HAL_ADC_AnalogWDGConfig(ADC_HandleTypeDef* hadc, ADC_AnalogWDGConfTypeDef* AnalogWDGConfig)
{
  /* Check the parameters */
  //ASSERT(IS_ADC_ALL_INSTANCE(hadc->Instance));
  ASSERT(IS_ADC_ANALOG_WATCHDOG_MODE(AnalogWDGConfig->WatchdogMode));
  ASSERT(IS_FUNCTIONAL_STATE(AnalogWDGConfig->ITMode));
  ASSERT(IS_ADC_RANGE(AnalogWDGConfig->HighThreshold));
  ASSERT(IS_ADC_RANGE(AnalogWDGConfig->LowThreshold));
  
  if((AnalogWDGConfig->WatchdogMode == ADC_ANALOGWATCHDOG_SINGLE_REG)     ||
     (AnalogWDGConfig->WatchdogMode == ADC_ANALOGWATCHDOG_SINGLE_INJEC)   ||
     (AnalogWDGConfig->WatchdogMode == ADC_ANALOGWATCHDOG_SINGLE_REGINJEC)  )
  {
    ASSERT(IS_ADC_CHANNEL(AnalogWDGConfig->Channel));
  }
...
HAL_StatusTypeDef HAL_ADCEx_InjectedConfigChannel(ADC_HandleTypeDef* hadc, ADC_InjectionConfTypeDef* sConfigInjected)
{
  ...
  /* Check the parameters */
  //ASSERT(IS_ADC_ALL_INSTANCE(hadc->Instance));
  ASSERT(IS_ADC_CHANNEL(sConfigInjected->InjectedChannel));
  ASSERT(IS_ADC_SAMPLE_TIME(sConfigInjected->InjectedSamplingTime));
  ASSERT(IS_FUNCTIONAL_STATE(sConfigInjected->AutoInjectedConv));
  ASSERT(IS_ADC_RANGE(sConfigInjected->InjectedOffset));
  
  if(hadc->Init.ScanConvMode != ADC_SCAN_DISABLE)
  {
    ASSERT(IS_ADC_INJECTED_RANK(sConfigInjected->InjectedRank));
    ASSERT(IS_ADC_INJECTED_NB_CONV(sConfigInjected->InjectedNbrOfConversion));
    ASSERT(IS_FUNCTIONAL_STATE(sConfigInjected->InjectedDiscontinuousConvMode));
  }
