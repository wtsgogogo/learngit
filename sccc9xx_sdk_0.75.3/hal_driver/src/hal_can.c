...
HAL_StatusTypeDef HAL_CAN_Init(CAN_HandleTypeDef* hcan)
{
    ...
    ASSERT(IS_CAN_MODE(hcan->Init.Mode));
    ASSERT(IS_CAN_SJW(hcan->Init.SJW));
    ASSERT(IS_CAN_BS1(hcan->Init.BS1));
    ASSERT(IS_CAN_BS2(hcan->Init.BS2));
    ASSERT(IS_CAN_PRESCALER(hcan->Init.Prescaler));
...
HAL_StatusTypeDef HAL_CAN_ConfigFilter(CAN_HandleTypeDef* hcan, CAN_FilterConfTypeDef* sFilterConfig)
{
    ...
    /* Check the parameters */
    ASSERT(IS_CAN_FILTER_NUMBER(sFilterConfig->FilterNumber));
    ASSERT(IS_CAN_FILTER_MODE(sFilterConfig->FilterMode));
    ASSERT(IS_CAN_FILTER_SCALE(sFilterConfig->FilterScale));
    ASSERT(IS_CAN_FILTER_FIFO(sFilterConfig->FilterFIFOAssignment));
...
static uint32_t HAL_CAN_Transmit_Mailbox_Config(CAN_HandleTypeDef* hcan)
{
    ...
    /* Check the parameters */
    ASSERT(IS_CAN_IDTYPE(hcan->pTxMsg->IDE));
    ASSERT(IS_CAN_RTR(hcan->pTxMsg->RTR));
    ASSERT(IS_CAN_DLC(hcan->pTxMsg->DLC));
