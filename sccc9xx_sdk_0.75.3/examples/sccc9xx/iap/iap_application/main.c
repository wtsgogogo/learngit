...
static const uint8_t uart_svc_uuid_128[] = {0x9e,0xca,0xdc,0x24,0x0e,0xe5,0xa9,0xe0,0x93,0xf3,0xa3,0xb5,0x01,0x00,0x40,0x6e};
static const uint8_t uart_rx_char_uuid_128[] = {0x9e,0xca,0xdc,0x24,0x0e,0xe5,0xa9,0xe0,0x93,0xf3,0xa3,0xb5,0x02,0x00,0x40,0x6e};
static const uint8_t uart_tx_char_uuid_128[] = {0x9e,0xca,0xdc,0x24,0x0e,0xe5,0xa9,0xe0,0x93,0xf3,0xa3,0xb5,0x03,0x00,0x40,0x6e};
...
static const struct att_decl uart_server_att_decl[UART_SVC_ATT_NUM] =
{
    [UART_SVC_IDX_RX_CHAR] = {
...
static void uart_server_init(void);
static void uart_server_timer_cb(void *param);
static void uart_init(void);
static void uart_server_read_req_ind(uint8_t att_idx, uint8_t con_idx);
static void uart_server_write_req_ind(uint8_t att_idx, uint8_t con_idx, uint16_t length, uint8_t const *value);
static void uart_server_send_notification(void);
...
static void uart_server_init(void)
{
    uart_server_timer_inst = builtin_timer_create(uart_server_timer_cb);
...
static void uart_server_write_req_ind(uint8_t att_idx, uint8_t con_idx, uint16_t length, uint8_t const *value)
{
    ...
    { 
        ...
        else
        {
            ...
            ASSERT(length <= UART_SVC_BUFFER_SIZE);
...
static void gatt_manager_callback(enum gatt_evt_type type,union gatt_evt_u *evt,uint8_t con_idx)
{
    if (connect_id != 0xff)
    {
        ...
        case SERVER_WRITE_REQ:
            LOG_I("write req");
            uart_server_write_req_ind(evt->server_write_req.att_idx, con_idx, evt->server_write_req.length, evt->server_write_req.value);
        break;
...
static void dev_manager_callback(enum dev_evt_type type,union dev_evt_u *evt)
{
    ...
    case ADV_OBJ_CREATED:
        ASSERT(evt->obj_created.status == 0);
