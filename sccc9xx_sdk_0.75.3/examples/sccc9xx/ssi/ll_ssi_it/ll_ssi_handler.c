...
void LL_SSI_IRQHandler(void)
{
    while (1)
    {
        if (LL_SSI_IsActiveFlagIT_Mask(SSI, SSI_ITIS_MST))
        {
            LOG_W("ssi0x%x,multi_master contention", SSI);
            SSI->MSTICR;
            ASSERT(0);
        }
        else if (LL_SSI_IsActiveFlagIT_Mask(SSI, SSI_ITIS_RXF))
        {
            ssi_rx_full_isr(SSI);
        }
        else if (LL_SSI_IsActiveFlagIT_Mask(SSI, SSI_ITIS_RXO))
        {
            LOG_W("ssi0x%x,Receive FIFO Overflow", SSI);
            SSI->RXOICR;
            ASSERT(0);
        }
        else if (LL_SSI_IsActiveFlagIT_Mask(SSI, SSI_ITIS_RXU))
        {
            LOG_W("ssi0x%x,Receive FIFO Underflow", SSI);
            SSI->RXUICR;
            ASSERT(0);
        }
        else if (LL_SSI_IsActiveFlagIT_Mask(SSI, SSI_ITIS_TXO))
        {
            LOG_W("si0x%x,Transmit FIFO Overflow", SSI);
            SSI->TXOICR;
            ASSERT(0);
        }
        else if (LL_SSI_IsActiveFlagIT_Mask(SSI, SSI_ITIS_TXE))
        {
            if (LL_SSI_IsEnabledDMAReq_TX(SSI))
            {
