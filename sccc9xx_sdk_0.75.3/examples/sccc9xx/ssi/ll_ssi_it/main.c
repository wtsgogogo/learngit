 ...
 * @brief   This sample code shows how to use SSI HAL API to transmit
 *          and receive a data buffer with a communication process based on
 *          Interrupt transfer.
 *          The communication is done using 1 SCCC9XX board and a SPI Flash module(W25Q128JV).
 ******************************************************************************
 */
...
static void ssi_test_init(void)
{
  ...
  LL_SSI_SetClockDivider(SSI, 128);
  LL_SSI_SetRxdSampleDelay(SSI, 0);
  LL_SSI_SetSSTE(SSI, 0);
}
...
int main(void)
{
 ...
 "aTxBuffer" buffer & receive data through "aRxBuffer" */
  if (LL_SSI_TransmitReceive_IT(SSI, (uint8_t *)ssi_tx_buf, (uint8_t *)ssi_rx_buf, 4) != SUCCESS)
  {
