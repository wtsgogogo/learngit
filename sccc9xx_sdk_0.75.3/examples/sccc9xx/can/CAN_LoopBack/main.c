...
bool can_polling_mode_flag = true;
...
int main(void)
{
    ...
    while(1)
    {
        if(can_polling_mode_flag)
        {
            CAN_SetMsg();
            HAL_CAN_Transmit(&Can_Handle,1000);
            Init_RxMes();
            if(HAL_CAN_Receive(&Can_Handle, CAN_FIFO0,1000)==HAL_OK)
            {
                LOG_I("\r\nCAN POLLING RECV DATA:");
                LOG_HEX(Can_Handle.pRxMsg->Data,8);
                flag++;
            }
